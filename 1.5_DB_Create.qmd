...
ymd_hms(file.info("./1.5_DB_Create.qmd")$ctime, tz = "America/Los_Angeles")
```
...
ymd_hms(file.info("./1.5_DB_Create.qmd")$atime, tz = "America/Los_Angeles")
```
...
standlist <- read_csv("./data/1_Import_Clean2.csv", 
                      col_types = "ccc")
...
tree <- tree |> mutate(
  Species = SPCD, 
  History = ifelse(STATUSCD == 1 & is.na(AGENTCD), 1, 8),
...
con <- dbConnect(RSQLite::SQLite(), dbname = "./data/homemade_fvs_db_17sept24.db")
...
# Remove NA values when writing files (FVS does NOT like them)
### UPDATE YOUR OUTPUT FILE NAMES HERE ###
# write.csv(tree_table, file = "data/tree_table_R1_Carbon_2014.csv", na = "", row.names = FALSE)
# write.csv(stands_with_fuels, file = "data/stand_table_R1_Carbon2014.csv", na = "", row.names = FALSE)
...
# write_csv(splt_cn, "./data/1.5_dbCreation_standlist.csv")
```
...
```{r}
# end1.5 <- read_csv("./data/1.5_dbCreation_standlist.csv")
# end1.0 <- read_csv("./data/1_Import_Clean2.csv", 
#                       col_types = "ccc")
